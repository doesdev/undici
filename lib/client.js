...
const kReconnects = Symbol('reconnects')
...
function reconnect (client, err) {
  ...
  const giveUp = client.maxRetries && client[kReconnects] >= client.maxRetries
  client[kCallbacks] = []
  client[kRequests] = []
  ...
  if (giveUp) {
    client.emit('maxRetriesExceeded', err)
    client.close()
  } else if (client[kQueue].length() > 0) {
    connect(client)
  }
